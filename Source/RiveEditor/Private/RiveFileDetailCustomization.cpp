...
void FRiveFileDetailCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{

	// Find the property you want to customize
	TSharedRef<IPropertyHandle> MyProperty = DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(URiveFile, Artboards));
	...
	{
		TSharedRef<IPropertyHandle> ElementHandle = MyProperty->GetChildHandle(Index).ToSharedRef();
				...
				[
					...
					[
						SNew(STextBlock)
							...
							.Text(FText::FromString(FString(TEXT("\xf247"))))
							.ToolTipText(FText::FromString(TEXT("Artboard")))
					]
					+ SHorizontalBox::Slot()
					.VAlign(VAlign_Center)
					.Padding(10.f, 0.f)
					...
					[
						...
						.Text(FText::FromString(Artboard->GetArtboardName()))
					]
				];
				
				for (auto StateMachineName : Artboard->GetStateMachineNames())
				{
					TUniquePtr<FRiveStateMachine> StateMachine = MakeUnique<FRiveStateMachine>(Artboard->GetNativeArtboard(), StateMachineName);
					uint32 StateMachineInputCount = StateMachine->GetInputCount();
					...
					[
						...
						[
							SNew(STextBlock)
								.Font(FAppStyle::Get().GetFontStyle("FontAwesome.11"))
								.Text(FText::FromString(FString(TEXT("\xf0e8"))))
								.ToolTipText(FText::FromString(TEXT("StateMachine")))
						]
						+ SHorizontalBox::Slot()
						.VAlign(VAlign_Center)
						.Padding(10.f, 0.f)
						...
						[
							...
							.IsReadOnly(true)
							.Font(DEFAULT_FONT("Mono", 8))
							.Text(FText::FromString(StateMachineName))
						]
					];

					
					for (uint32 InputI = 0; InputI < StateMachineInputCount; InputI++)
					{
						rive::SMIInput* Input = StateMachine->GetInput(InputI);
						FString TypeString = RiveArtboardDetailCustomizationPrivate::GetTypeStringForInput(Input);
						FLinearColor TypeColor = RiveArtboardDetailCustomizationPrivate::GetColorForInput(Input);
							...
							[
								...
								+ SHorizontalBox::Slot()
									...
									[
										...
										.Font(DEFAULT_FONT("Mono", 8))
										.ColorAndOpacity(TypeColor)
										.Text(FText::FromString(TypeString)) //FText::FromString(ElementHandle->GetProperty()
									]
								+ SHorizontalBox::Slot()
									.VAlign(VAlign_Center)
									.Padding(5.f, 0.f)
									...
									[
										...
										.IsReadOnly(true)
										.Font(DEFAULT_FONT("Mono", 8))
										.Text(FText::FromString(InputName)) //FText::FromString(ElementHandle->GetProperty()
									]
							];
					}
				}
		...
		// [
		// 	SNew(STextBlock)
		// 	.Text(FText::FromString(TEXT("Hi!!!"))) //FText::FromString(ElementHandle->GetProperty()
		// // 	SNew(SHorizontalBox)
		// // 	+ SHorizontalBox::Slot()
		// // 	.AutoWidth()
		// // 	// [
	...
	// TSharedRef<IPropertyHandle> MyArrayProperty = DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(URiveFile, Artboards));
	...
	// for (uint32 Index = 0; Index < NumElements; ++Index)
	// {
	// 	TSharedRef<IPropertyHandle> ElementHandle = MyArrayProperty->GetChildHandle(Index).ToSharedRef();
	//
	// 	
	//
	// 	
