...
UEnum* FRiveBlueprintGeneration::GenerateEnum(
    ...
    bool bSave)
{
    ...
    UPackage* FolderPackage = FindPackage(nullptr, *SanitizedPackageName);
    if (!FolderPackage && FPackageName::DoesPackageExist(SanitizedPackageName))
    {
        FolderPackage = UPackageTools::LoadPackage(*SanitizedPackageName);
    }
    if (!FolderPackage)
    {
    ...
    if (bSave)
    {
        ...
        // Don't crash the editor if we can't save. The user can still save the
        // file themselves afterwords.
...
UBlueprint* FRiveBlueprintGeneration::GenerateViewModelBlueprint(
    const FString& FolderPath,
...
void FRiveBlueprintGeneration::GenerateForFile(URiveFile* RiveFile,
                                               const FString& FolderPath,
                                               bool bSave)
{
    ...
    for (size_t i = 0; i < RiveFile->EnumDefinitions.Num(); i++)
    {
        ...
        GeneratedEnums[i] = GenerateEnum(FolderPath, EnumDefinition, bSave);
    }
    ...
    {
        if (auto GeneratedBlueprint = GenerateViewModelBlueprint(FolderPath,
                                                                 ViewModel,
                                                                 GeneratedEnums,
                                                                 bSave);
