...
URiveTextureObject* FRiveObjectFactory::CreateRiveObject()
{
	...
	const FString FolderName = *FPackageName::GetLongPackagePath(RivePackageName);
	
	const FString BaseWidgetPackageName = FolderName + TEXT("/") + RiveFile->GetName();
	...
	AssetToolsModule.Get().CreateUniqueAssetName(BaseWidgetPackageName, FString("_Texture"), /*out*/ RiveObjectPackageName, /*out*/ RiveObjectName);
