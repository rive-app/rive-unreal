...
void URiveArtboard::Initialize(rive::File* InNativeFilePtr, int32 InIndex, const FString& InStateMachineName)
{
    ...
    if (InIndex >= InNativeFilePtr->artboardCount())
    {
        ...
        UE_LOG(LogRive, Warning, TEXT("Artboard index specified is out of bounds, using the last available artboard index instead, which is %d"), InIndex);
    }
...
void URiveArtboard::Initialize(rive::File* InNativeFilePtr, const FString& InName, const FString& InStateMachineName)
{
    ...
    if (!NativeArtboard)
    {
        UE_LOG(LogRive, Error, TEXT("Could not initialize the artboard by the name '%s'. Initializing with default artboard instead"), *InName);
