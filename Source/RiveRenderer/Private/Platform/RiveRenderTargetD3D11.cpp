...
#if WITH_RIVE
	rive::gpu::RenderContext* RenderContext = RiveRenderer->GetRenderContextPtr();
...
#if WITH_RIVE
		if (CachedRenderTargetD3D)
		{
			CachedRenderTargetD3D.reset();
		}
		// For now we just set one renderer and one texture
		rive::gpu::RenderContextD3DImpl* const RenderContextD3DImpl = RenderContext->static_impl_cast<rive::gpu::RenderContextD3DImpl>();
		CachedRenderTargetD3D = RenderContextD3DImpl->makeRenderTarget(Desc.Width, Desc.Height);
		CachedRenderTargetD3D->setTargetTexture(D3D11ResourcePtr);
#endif // WITH_RIVE
	}
}
...
rive::rcp<rive::gpu::RenderTarget> FRiveRenderTargetD3D11::GetRenderTarget() const
{
	return CachedRenderTargetD3D;
}
