                                               ...
                                               VERTEX_CONTEXT_DECL)
{
    ...
    uint4 tessVertexData = TEXEL_FETCH(_EXPORTED_tessVertexTexture, tess_texel_coord(tessVertexIdx));
    uint contourIDWithFlags = tessVertexData.w;

    // Fetch and unpack the contour referenced by the tessellation vertex.
    uint4 contourData = STORAGE_BUFFER_LOAD4(_EXPORTED_contourBuffer, contour_data_idx(contourIDWithFlags));
    ...
    float2x2 M = make_float2x2(uintBitsToFloat(STORAGE_BUFFER_LOAD4(_EXPORTED_pathBuffer, o_pathID * 2u)));
    uint4 pathData = STORAGE_BUFFER_LOAD4(_EXPORTED_pathBuffer, o_pathID * 2u + 1u);
    ...
    if (localVertexID != vertexIDOnContour)
    {
        ...
        if ((replacementTessVertexData.w & 0xffffu) != (contourIDWithFlags & 0xffffu))
        {
            ...
            {
                tessVertexData =
                    TEXEL_FETCH(_EXPORTED_tessVertexTexture, tess_texel_coord(int(vertexIndex0)));
            }
        }
        else
        {
    ...
    if (strokeRadius != .0) // Is this a stroke?
    {
        ...
        if (joinType != 0u)
        {
            ...
            uint4 otherJoinData = TEXEL_FETCH(_EXPORTED_tessVertexTexture, otherJoinTexelCoord);
