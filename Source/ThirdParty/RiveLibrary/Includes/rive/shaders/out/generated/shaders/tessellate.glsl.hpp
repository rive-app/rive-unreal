...
ATTR(3, uint, _EXPORTED_a_args_a);
ATTR(4, uint, _EXPORTED_a_args_b);
...
VERTEX_MAIN(_EXPORTED_tessellateVertexMain, Attrs, attrs, _vertexID, _instanceID)
{
    ...
    float2 p0 = _EXPORTED_a_p0p1_.xy;
    float2 p1 = _EXPORTED_a_p0p1_.zw;
    float2 p2 = _EXPORTED_a_p2p3_.xy;
    float2 p3 = _EXPORTED_a_p2p3_.zw;
    ...
    float y = isFirstSpan ? _EXPORTED_a_joinTan_and_ys.z : _EXPORTED_a_joinTan_and_ys.w;
    int x0x1 = int(isFirstSpan ? _EXPORTED_a_args.x : _EXPORTED_a_args.y);
#ifdef GLSL
    int x1up = x0x1 << 16;
    if (_EXPORTED_a_args.z == 0xffffffffu)
    {
    ...
    if ((contourIDWithFlags & CULL_EXCESS_TESSELLATION_SEGMENTS_CONTOUR_FLAG) != 0u)
    {
        ...
        uint pathIDBits =
            STORAGE_BUFFER_LOAD4(_EXPORTED_contourBuffer, contour_data_idx(contourIDWithFlags)).z;
        float2x2 mat =
            make_float2x2(uintBitsToFloat(STORAGE_BUFFER_LOAD4(_EXPORTED_pathBuffer, pathIDBits * 2u)));
    ...
    {
        float2x2 joinTangents = float2x2(tangents[1], _EXPORTED_a_joinTan_and_ys.xy);
...
FRAG_DATA_MAIN(uint4, _EXPORTED_tessellateFragmentMain)
{
    ...
    else
    {
        ...
        else
        {
            ...
            polarT = (root.y != .0) ? root.x / root.y : .0;
